...
class sfp_pastebin(SpiderFootPlugin):
    ...
    def handleEvent(self, event):
        ...
        for dom in list(self.domains.keys()):
            ...
            relevant_links = [link for link in new_links if self.sf.urlBaseUrl(link).endswith(target)]
            ...
            for link in relevant_links:
                ...
                if (
                    re.search(r"[^a-zA-Z\-\_0-9]" + re.escape(eventData) + r"[^a-zA-Z\-\_0-9]", res["content"], re.IGNORECASE)
                    is None
                ):
                    continue
