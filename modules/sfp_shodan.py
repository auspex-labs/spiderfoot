...
class sfp_shodan(SpiderFootPlugin):
    ...
    def searchHosts(self, qry):
        res = self.sf.fetchUrl(
            "https://api.shodan.io/shodan/host/search?query=hostname:" + qry + "&key=" + self.opts["api_key"],
            timeout=self.opts["_fetchtimeout"],
            useragent="SpiderFoot",
        )
        time.sleep(1)
    ...
    def handleEvent(self, event):
        ...
        for addr in qrylist:
            ...
            if rec.get("os") is not None:
                ...
                evt = SpiderFootEvent("OPERATING_SYSTEM", rec.get("os") + " (" + addr + ")", self.__name__, pevent)
                self.notifyListeners(evt)

            if rec.get("devtype") is not None:
                # Notify other modules of what you've found
                evt = SpiderFootEvent("DEVICE_TYPE", rec.get("devtype") + " (" + addr + ")", self.__name__, pevent)
                self.notifyListeners(evt)
