    ...
    def fetchRir(self, url):
        if url in self.memCache:
            ...
            if res["content"] is not None and res["code"] != "404":
                ...
                evt = SpiderFootEvent("RAW_RIR_DATA", res["content"], self.__name__, self.currentEventSrc)
            ...
            if "pocs" in ret:
                if "pocRef" in ret["pocs"]:
                    ...
                    for p in ref:
                        ...
                        evt = SpiderFootEvent("RAW_RIR_DATA", "Possible full name: " + name, self.__name__, self.currentEventSrc)
                        self.notifyListeners(evt)
