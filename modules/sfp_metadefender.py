...
class sfp_metadefender(SpiderFootPlugin):
    ...
    def queryDomain(self, qry):
        ...
        res = self.sf.fetchUrl(
            "https://api.metadefender.com/v4/domain/" + qry, headers=headers, timeout=15, useragent=self.opts["_useragent"]
        )
    ...
    def queryIp(self, qry):
        headers = {"Accept": "application/json", "apikey": self.opts["api_key"]}
        res = self.sf.fetchUrl(
            "https://api.metadefender.com/v4/ip/" + qry, headers=headers, timeout=15, useragent=self.opts["_useragent"]
        )
