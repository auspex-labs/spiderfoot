...
class sfp_bgpview(SpiderFootPlugin):
    ...
    def producedEvents(self):
        return ["BGP_AS_MEMBER", "NETBLOCK_MEMBER", "PHYSICAL_ADDRESS", "RAW_RIR_DATA"]
    ...
    def queryIp(self, qry):
        res = self.sf.fetchUrl(
            "https://api.bgpview.io/ip/" + qry, useragent=self.opts["_useragent"], timeout=self.opts["_fetchtimeout"]
        )
    ...
    def queryNetblock(self, qry):
        res = self.sf.fetchUrl(
            "https://api.bgpview.io/prefix/" + qry, useragent=self.opts["_useragent"], timeout=self.opts["_fetchtimeout"]
        )
