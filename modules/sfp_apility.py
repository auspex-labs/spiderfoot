...
class sfp_apility(SpiderFootPlugin):
    ...
    meta = {
        ...
        "dataSource": {
            ...
            "references": ["https://auth0.com/signals/docs/"],
            "apiKeyInstructions": [
    ...
    def producedEvents(self):
        return ["MALICIOUS_IPADDR", "MALICIOUS_INTERNET_NAME", "IP_ADDRESS", "PROVIDER_MAIL", "PROVIDER_DNS", "RAW_RIR_DATA"]
    ...
    def queryBadDomain(self, qry):
        ...
        res = self.sf.fetchUrl(
            "https://api.apility.net/baddomain/" + qry, headers=headers, timeout=15, useragent=self.opts["_useragent"]
        )
    ...
    def queryBadIp(self, qry):
        headers = {"Accept": "application/json", "X-Auth-Token": self.opts["api_key"]}
        res = self.sf.fetchUrl(
            "https://api.apility.net/badip/" + qry, headers=headers, timeout=15, useragent=self.opts["_useragent"]
        )
    ...
    def queryIp(self, qry):
        headers = {"Accept": "application/json", "X-Auth-Token": self.opts["api_key"]}
        res = self.sf.fetchUrl(
            "https://api.apility.net/v2.0/ip/" + qry, headers=headers, timeout=15, useragent=self.opts["_useragent"]
        )
    ...
    def queryAsIp(self, qry):
        headers = {"Accept": "application/json", "X-Auth-Token": self.opts["api_key"]}
        res = self.sf.fetchUrl(
            "https://api.apility.net/v2.0/as/ip/" + qry, headers=headers, timeout=15, useragent=self.opts["_useragent"]
        )
