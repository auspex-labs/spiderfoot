...
class sfp_crxcavator(SpiderFootPlugin):
    ...
    def producedEvents(self):
        return ["APPSTORE_ENTRY", "INTERNET_NAME", "LINKED_URL_INTERNAL", "AFFILIATE_INTERNET_NAME", "PHYSICAL_ADDRESS"]

    def query(self, qry):
        params = urllib.parse.urlencode({"q": qry.encode("raw_unicode_escape").decode("ascii", errors="replace")})

        res = self.sf.fetchUrl(
            f"https://api.crxcavator.io/v1/search?{params}", useragent=self.opts["_useragent"], timeout=self.opts["_fetchtimeout"]
        )
